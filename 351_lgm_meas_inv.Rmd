...
In particolare, le prime quattro verifiche si basano sulle covarianze (~~) piuttosto che sulle regressioni (~) per descrivere le relazioni tra variabili. Nella quinta verifica, si esamina un CLPM che ipotizza relazioni causali direzionali in funzione del tempo (modello di regressione) -- ad esempio, i dati del giorno 7 prevedono i dati del giorno 8. Vale la pena notare che le relazioni tra le variabili prese in esame nelle prime quattro verifiche sono più simili a correlazioni bivariate, mentre l'ultima verifica è un test sulle relazioni causali. 
...
Dopo avere definito il modello, lo adattiamo ai dati. Per questa e tutte le analisi successive, aggiungiamo la riga `std.lv = TRUE` per tenere conto della nostra modifica alle impostazioni sul vincolo dei default di `lavaan` che garantisce l'identificabilità del modello. Esaminiamo i risultati con `summary()`.
...
Dopo avere costruito il modello "baseline" possiamo progressivamente introdurre vincoli aggiuntivi e confrontare le versioni nidificate dei modelli proposti. Dopo aver adattato tutti i modelli, confronteremo i loro indici di adattamento e selezioneremo il modello più appropriato. 
...
Passiamo ora ad applicare l'insieme più stringente di vincoli al modello di invarianza degli errori. 

### Definizione del modello

In questo modello, oltre ai vincoli applicati al modello scalare, imponiamo il vincolo dell'eguaglianza dei termini d'errore nelle diverse rilevazioni temporali. Ancora una volta, questo risultato viene ottenuto applicando una singola etichetta ai gruppi di item che desideriamo vincolare (ad esempio `psp1.7 ~~ psp1u*psp1.7`). Nella sintassi, questo è simile a quello che abbiamo usato in precedenza con le covarianze; tuttavia, questa volta consideriamo la covariazione di ogni variabile con se stessa. 
...
Il modello ha un adattamento accettabile. I valori $R^2$ variano da .38 a .63. La figura \@ref(fig:parintclpm) mostra che l'autopresentazione perfezionistica del giorno precedente predice fortemente l'autopresentazione perfezionista del giorno successivo. Allo stesso modo, l'ansia sociale del giorno precedente predice fortemente l'ansia sociale del giorno successivo. Nel contempo, il percorso incrociato tra ciascuna variabile latente e la variabile opposta è più debole, anche se ancora statisticamente significativo. Questo tipo di pattern (grandi percorsi auto-regressivi, piccoli percorsi incrociati) è tipico della maggior parte dei modelli Cross-lagged Panel.
...
Questo modello apporta quattro modifiche principali al modello precedente: 
...
- le medie latenti per tutte le variabili latenti dal secondo momento temporale in poi sono parametri che possono variare liberamente. 

La figura \@ref(fig:riclpm) fornisce una rappresentazione grafica di questo modello.
