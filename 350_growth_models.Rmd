...
Nella loro forma più semplice i modelli LGM sono equivalenti a quelli che, nel contesto dell'analisi di regressione, si chiamano modelli a effetti misti (mixed-effects models). Il modello lineare standard assume osservazioni indipendenti, mentre la caratteristica fondamentale dei modelli mixed-effects è il fatto che le osservazioni sono ragruppate in cluster -- per esempio, possiamo avere misure ripetute per lo stesso individuo.

Un modo per affrontare problemi di correlazione tra osservazioni raggruppate è utilizzare i modelli a effetti misti. Questi modelli combinano effetti fissi, ovvero coefficienti di regressione standard, con effetti casuali che consentono a ciascun gruppo di avere un effetto unico. Questo effetto unico è considerato una deviazione casuale dall’intercetta e dalle pendenze complessive. I modelli misti rappresentano un equilibrio tra ignorare completamente gli effetti unici dei gruppi e sovra-enfatizzarli eseguendo regressioni separate per ogni gruppo.
...
ranef_latent = data.frame(coef(mix_mod)[[1]], lavPredict(growth_curve_model)) %>% 
